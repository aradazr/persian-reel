...
اگر تا حالا با این چیزها کار نکردی، نگران نباش. این بخش فرض می‌کنه هیچی نمی‌دونی.

</div>

<div dir="rtl">

## ۱. اصلاً «اسکیل» یعنی چی؟

کلاد یک دستیار هوش مصنوعیه. **اسکیل** یعنی یک بستهٔ دستورالعمل که بهش یاد می‌ده یک
کار خاص رو چطور انجام بده.

مثل این می‌مونه که به یک آشپز ماهر که آشپزی بلده ولی غذای ایرانی بلد نیست، دفترچهٔ
دستور قرمه‌سبزی بدی. از اون به بعد هر وقت بگی «قرمه‌سبزی می‌خوام»، خودش می‌دونه چیکار کنه.

این اسکیل، دفترچهٔ **«ویدیوی سلفی رو تبدیل کن به ریلز»** ـه.
...
هرجایی که یک دستیار هوش مصنوعی به **کامپیوتر خودت** دسترسی داشته باشه. دلیلش
اینه که این اسکیل باید واقعاً فایل ویدیو رو باز کنه، ببره، و رندر بگیره.

</div>

| | | |
|---|---|---|
| ✅ | **Claude Code** | جایی که ساخته و تست شده |
| ✅ | **اپلیکیشن Claude** مک و ویندوز | همون Claude Code داخلشه |
| ✅ | **Codex** | فرمت اسکیل یکیه، فقط یک خط تنظیم می‌خواد ↓ |
| ⚠️ | **claude.ai یا chatgpt.com** توی مرورگر | به کامپیوتر تو دسترسی ندارن، پس ابزارهای ویدیو اونجا نیستن |

<div dir="rtl">

### راه‌اندازی برای Codex

Codex از فایلی به اسم `AGENTS.md` دستور می‌گیره. اسکیل رو در مسیر مشترک ایجنت‌ها
بذار و یک اشاره بهش اضافه کن:

</div>

```bash
git clone https://github.com/aradazr/persian-reel ~/.agents/skills/persian-reel

cat >> ~/.codex/AGENTS.md <<'EOF'

## ساخت ریلز فارسی
وقتی کاربر ویدیوی تاکینگ‌هد داد و خواست ادیت/زیرنویس/ریلز بشه،
اول ~/.agents/skills/persian-reel/SKILL.md را بخوان و دنبالش کن.
EOF
```

<div dir="rtl">

همین. از این به بعد Codex هم مثل کلاد می‌دونه چیکار کنه.
...
فقط مک و لینوکس. روی ویندوز باید WSL نصب کنی که خارج از حوصلهٔ این راهنماست.

**قدم اول — ترمینال رو باز کن.**
ترمینال یک پنجرهٔ سیاهه که توش دستور تایپ می‌کنی. روی مک، `Command + Space` بزن،
بنویس `Terminal` و اینتر.

**قدم دوم — دستیارت رو نصب کن.** یکی از این دو خط رو کپی کن، توی ترمینال بچسبون،
اینتر بزن:

</div>

```bash
npm install -g @anthropic-ai/claude-code   # کلاد
npm install -g @openai/codex               # یا Codex
```

<div dir="rtl">

اگر گفت `npm: command not found`، یعنی Node.js نداری. از [nodejs.org](https://nodejs.org)
نصبش کن و دوباره امتحان کن.

**قدم سوم — ابزارهای ویدیو رو نصب کن.** این‌ها کارهای واقعی برش و رندر رو انجام می‌دن:
...
اگر `brew` نداشتی، اول از [brew.sh](https://brew.sh) نصبش کن.
گوگل کروم هم باید روی سیستمت باشه — احتمالاً هست.

**قدم چهارم — این اسکیل رو بگیر:**
...
**قدم پنجم — یک فونت فارسی بذار.** توی ریپو نیست چون حق نشرش مال خودم نیست.
[وزیرمتن](https://github.com/rastikerdar/vazirmatn) رایگانه و کارت رو راه می‌ندازه.
...
## ۴. حالا چطور استفاده کنم؟

توی ترمینال برو داخل پوشه‌ای که ویدیوت اونجاست و ابزارت رو باز کن — `claude` یا
`codex`. بعد فقط **عین یک آدم معمولی** باهاش حرف بزن:

</div>

> <div dir="rtl"><b>«فایل video.mov رو برام ادیت کن، دربارهٔ آموزش برنامه‌نویسیه»</b></div>

<div dir="rtl">

همین. لازم نیست اسم اسکیل رو بگی، لازم نیست دستور فنی بلد باشی — خودش می‌فهمه باید
از این اسکیل استفاده کنه.

بار اول حدود ده دقیقه طول می‌کشه چون باید مدل تشخیص گفتار (سه گیگابایت) رو دانلود کنه.
دفعات بعد سریعه.

</div>

> [!TIP]
> <div dir="rtl"><b>یک جمله دربارهٔ موضوع ویدیو بگو.</b> فرقش زیاده. اگر بگی «دربارهٔ آشپزیه»، گرافیک‌هایی که می‌سازه به آشپزی ربط پیدا می‌کنن. اگر چیزی نگی، از روی حرف‌های خودت حدس می‌زنه.</div>

<div dir="rtl">

## ۵. چطور بهتر فیلم بگیرم؟

</div>

> [!IMPORTANT]
> <div dir="rtl"><b>پایین کادر بشین و از دوربین دورتر.</b><br>نصف بالای تصویر قراره با گرافیک پوشیده بشه. اگر وسط کادر باشی، مجبوریم برشت بدیم و دست‌هات از تصویر بیرون می‌افتن.<br>سرت باید حدود دوسوم پایین کادر باشه، نه بالا.</div>

<div dir="rtl">

بقیه‌ش مهم نیست. نور معمولی اتاق کافیه، میکروفون خاصی لازم نیست.
...
| مشکل | چیکار کنی |
|---|---|
| ویدیوی خروجی کاملاً سیاهه | بگو «ویدیو سیاه دراومده» — علتش نوشته شده و پیداش می‌کنه |
| زیرنویس‌ها غلط املایی دارن | طبیعیه، تشخیص گفتار کامل نیست. بگو «این کلمه رو درست کن» |
| خطای `command not found` | یعنی یکی از برنامه‌های قدم دوم یا سوم نصب نشده |
| خیلی کند رندر می‌گیره | بار اول عادیه چون مدل دانلود می‌شه |

<div dir="rtl">

و اگر چیزی خراب بود، **فقط بگو چی اشتباهه.** لازم نیست بدونی کجای کد مشکل داره —
همین که بگی «ساند افکت‌ها درست نیستن» کافیه که خودش بگرده و پیداش کنه.
