...
| | | |
|---|---|---|
| ✅ | **Claude Code** | جایی که ساخته و تست شده. **این رو استفاده کن.** |
| ✅ | **اپلیکیشن Claude برای مک و ویندوز** | همون Claude Code رو داخل خودش داره |
| ⚠️ | **claude.ai توی مرورگر** | اسکیل رو قبول می‌کنه ولی ابزارهای ویدیو اونجا نیستن |
| ❌ | **ChatGPT** | اصلاً اسکیل نداره — پایین توضیح دادم |

> [!CAUTION]
> <div dir="rtl"><b>چرا توی ChatGPT کار نمی‌کنه؟</b><br>دو دلیل. اول اینکه فرمت اسکیل مال کلاده و ChatGPT همچین چیزی نداره. دوم و مهم‌تر: این اسکیل باید روی <b>کامپیوتر خودت</b> برنامه‌های ویرایش ویدیو رو اجرا کنه. ChatGPT به کامپیوتر تو دسترسی نداره.<br>اسکریپت‌های داخل این ریپو پایتون ساده‌ان و می‌تونی دستی اجراشون کنی، ولی اون کار دیگه‌ایه.</div>
...
**قدم دوم — کلاد رو نصب کن.** این خط رو کپی کن، توی ترمینال بچسبون، اینتر بزن:
...
توی ترمینال برو داخل پوشه‌ای که ویدیوت اونجاست و بنویس `claude`. بعد فقط
**عین یک آدم معمولی** باهاش حرف بزن:
...
همین. لازم نیست اسم اسکیل رو بگی، لازم نیست دستور فنی بلد باشی. کلاد خودش می‌فهمه
باید از این اسکیل استفاده کنه.
...
| مشکل | چیکار کنی |
|---|---|
| ویدیوی خروجی کاملاً سیاهه | به کلاد بگو «ویدیو سیاه دراومده» — علتش رو می‌دونه |
...
| خیلی کند رندر می‌گیره | بار اول عادیه. اگر همیشگیه به کلاد بگو |

<div dir="rtl">

و اگر چیزی خراب بود، **فقط بهش بگو چی اشتباهه.** لازم نیست بدونی کجای کد مشکل داره —
همین که بگی «ساند افکت‌ها درست نیستن» کافیه که خودش پیدا و درستش کنه.
